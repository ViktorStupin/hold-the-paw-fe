...
export const StepTwo = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  const LEFT_GRIDS = [
    {
      name: 'status' as keyof PetProfileFormValues,
      options: PET_STATUS,
    ...
    {
      name: 'age' as keyof PetProfileFormValues,
      options: PET_AGE,
  ...
  const RIGHT_GRIDS = [
    {
      name: 'breed' as keyof PetProfileFormValues,
      options: PET_BREED,
      labels: PET_BREED_LABEL_UA,
      layout: isMobile ? 'special-2' : ('grid-3' as LayoutType),
    },
    {
      name: 'size' as keyof PetProfileFormValues,
      options: PET_SIZE,
